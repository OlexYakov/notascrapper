...
def login() -> Tuple[bool, Response]:
    # GET infor page
    r = requests.get(infor_login_url)
...
def navigate_subjects_page() -> Tuple[bool, Response]:
    # Get list of classes
    r = requests.get(infor_insc_turmas_url, headers=headers)
    if r.status_code != 200:
        print(r.status_code)
        return False, None
    page = BeautifulSoup(r.content, 'html.parser')
...
def gen_classes_configs(subjects: List[Subject]):
    ...
    for d in [d for d in subjects if d.url != None]:
        ...
        r = requests.get(d.url, headers=headers)
        if r.status_code != 200:
...
def do_register(subjects: List[Subject]):
    ...
    for d in [d for d in subjects if d.url != None]:
        ...
        if len(fields) != 0:
            ...
            # r = requests.post(form_url, data=fields,
            #                   headers=headers )
...
if __name__ == "__main__":
    ...
    success, res = navigate_subjects_page()
    if not success:
